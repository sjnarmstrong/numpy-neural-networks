...
for dataset, datasetHiddenLayers in zip(datasets, hiddenLayers):
    ...
    for currentHiddenLayer in datasetHiddenLayers:
        ...
        for Trial in range(NumTrials):   
            ...
            BNN=batchNeuralNetwork(l, PopulationSize,lowerBound = -2, upperBound = 2)
            pso=PSO(1.49618,1.49618,0.7298,BNN.weights,BNN.getBatchError,-2,2)
            TestBNN=batchNeuralNetwork(l,PopulationSize,pso.bestPos)
            ...
            while epoch < generationCount:
                ...
                pso.iterate(TrainingInputs,TrainingOutputs,BNN.weights)
                
                TrainingError=errorScaleTr*np.sum((ThreshFunction2(TestBNN.forwardPropagateBest(TrainingInputs[0], pso.globalbestIndex))-TrainingOutputs[0])**2, axis=(0,1))
                TrErrors[Trial].append(TrainingError)    
                VError=errorScaleV*np.sum((ThreshFunction2(TestBNN.forwardPropagateBest(ValInputs[0], pso.globalbestIndex))-ValOutputs[0])**2, axis=(0,1))
                VErrors[Trial].append(VError)      
                TestError=errorScaleV*np.sum((ThreshFunction2(TestBNN.forwardPropagateBest(TestInputs[0], pso.globalbestIndex))-TestOutputs[0])**2, axis=(0,1))     
                TeErrors[Trial].append(TestError)
